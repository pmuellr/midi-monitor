...
function getCliParams(argv) {
  ...
  const columns = args['32'] ? 32 : 16
  const inputOnly = !!args['input-only']
  const outputOnly = !!args['output-only']
  ...
  if (args.list) return { opts, listPorts: true, columns, inputOnly, outputOnly }
  ...
  return { opts, aMidiDevice, vMidiDevice, columns, inputOnly, outputOnly }
}
...
options:
    -a --active        handle active sensing messages
    -s --sysex         handle sysex messages
    -t --timing        handle timing messages
    -l --list          list available midi ports
    -i --input-only    only print the MIDI input (via virtual-midi-port)
    -o --output-only   only print the MIDI output from midi-port
       --32            print hex in 32 byte columns instead of 16
